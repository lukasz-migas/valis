...
class Valis(object):
        ...
        for f in full_path_list:
            if os.path.isdir(f):
                ...
                if is_round:
                    ...
                    if len(matching_f) == 1:
                        if matching_f[0] not in self.original_img_list:
    ...
    def micro_rigid_register(self):
        ...
        for slide_name, slide_obj in self.slide_dict.items():
            ...
            rigid_img_obj.match_dict[
                fixed_rigid_obj
            ].matched_kp1_xy = slide_obj.xy_matched_to_prev
            rigid_img_obj.match_dict[
                fixed_rigid_obj
            ].matched_kp2_xy = slide_obj.xy_in_prev
            rigid_img_obj.match_dict[
                fixed_rigid_obj
            ].n_matches = slide_obj.xy_in_prev.shape[0]

            fixed_rigid_obj.match_dict[
                rigid_img_obj
            ].matched_kp1_xy = slide_obj.xy_in_prev
            fixed_rigid_obj.match_dict[
                rigid_img_obj
            ].matched_kp2_xy = slide_obj.xy_matched_to_prev
            fixed_rigid_obj.match_dict[
                rigid_img_obj
            ].n_matches = slide_obj.xy_in_prev.shape[0]
        ...
        for moving_idx, fixed_idx in self.iter_order:
            ...
            if moving_slide.image.ndim == 3 and moving_slide.is_rgb:
                moving_draw_img = warp_tools.resize_img(
                    moving_slide.image, moving_slide.processed_img.shape[0:2]
                )
            else:
            ...
            if fixed_slide.image.ndim == 3 and fixed_slide.is_rgb:
                fixed_draw_img = warp_tools.resize_img(
                    fixed_slide.image, fixed_slide.processed_img.shape[0:2]
                )
            else:
    ...
    def warp_and_save_slides(
        ...
        src_f_list = [
            self.original_img_list[slide_obj.stack_idx]
            for slide_obj in self.slide_dict.values()
        ]
